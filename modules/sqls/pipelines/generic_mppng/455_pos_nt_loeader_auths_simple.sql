...
v_institution_number varchar(8) := '&institution_number';
v_client_number_all Varchar2(120) := '&client_number';
v_amount varchar(8) := '&amount';
v_card_number varchar(20) := '&card_number';
v_date Varchar2(8) := '&record_date';
v_auth_code varchar(6) := '&auth_code';
v_retrieval_reference varchar(20) := '&retrieval_reference';
v_external_reference varchar(20);
v_retrieval_reference_number number;

v_client_number varchar(8);
v_amt varchar(15);
v_transaction_link_id varchar(30) := '';
...
Begin
	FOR curClientNumber in (WITH DATA AS ( SELECT v_client_number_all str FROM dual) SELECT trim(COLUMN_VALUE) str FROM DATA, xmltable(('"' || REPLACE(REPLACE(REPLACE(str, ' ', ''), ',', '","'), '|', '","') || '"'))) LOOP
		...
		if (v_card_number is null or v_card_number = '') then
			v_card_number := '4999224500000014';
		end if;
		...
		if (v_retrieval_reference is null or v_retrieval_reference = '') then
			...
			--v_retrieval_reference := '287155107001';
		end if;

		if (v_external_reference is null or v_external_reference = '') then
			v_external_reference := v_retrieval_reference;
			--v_external_reference := '1234567890123456789004';
		end if;
		...
		if (v_transaction_link_id is null or v_transaction_link_id = '') then
			...
			-- v_transaction_link_id := '6349694b-0d82559bb8-78abcd';
		end if;

		IF (v_errors is null or v_errors = '') THEN
			DBMS_OUTPUT.put_line('Insert data: terminal_id:' || v_terminal_id || ', amount: ' || v_amount || ', ' || v_amt || ', v_date: ' || v_date || ', trn_link_id: ' || v_transaction_link_id || ', rrn: ' || v_retrieval_reference || ', v_i: ' || v_i);
			...
			merchant_id = '0000000' || v_client_number,
			...
			where transaction_link_id in (v_transaction_link_id);
			commit;
			...
			VALUES (V_BPR_LOG_ID +1,to_timestamp(v_date || '00:00:00','YYYYMMDD HH24.MI.SS'),v_date,'15:51:09','BPR_HST_ISS',v_institution_number,'0200',to_date(v_date , 'YYYYMMDD'),to_date(v_date , 'YYYYMMDD'),v_retrieval_reference,'093687','00000000999','00000000000','999',NULL,NULL,v_card_number,'2212','000000','5999', v_card_number || '=2212221',v_terminal_id,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'978',v_amount,NULL,NULL,to_date(v_date , 'YYYYMMDD'),NULL,NULL,'901','00','00DE52:N/DE48.43:N/AVS_DATA:N/DE63.PY:N/DE63.AR:N/DE63.LG:N/DE63.CA:N/DE63.AD:N/DE63.IT:N/DE63.IA:N/DE63.GS:N/ECOM:210/DE60:50000800/',NULL,NULL,NULL,'0000000' || v_client_number,'BUNIT MERCHANT           BUNIT CITY   GB',NULL,NULL,NULL,NULL,v_external_reference,NULL,NULL,NULL,NULL,v_transaction_link_id,v_transaction_link_id,'RS2\NAFIDH-A557');
			INSERT INTO COS_BPR_DATA (BPR_LOG_ID,RECORD_DATE,BW3_RECORD_DATE,BW3_RECORD_TIME,BPR_TYPE_NAME,INSTITUTION_NUMBER,MESSAGE_TYPE,TRANSACTION_DATE,TRANSMISSION_DATE,RETRIEVAL_REFERENCE,STAN,ACQUIRING_INST_ID,FORWARDING_INST_ID,TRANSACTION_STATUS,NETWORK_ID,BATCH_ID,CARD_NUMBER,EXPIRY_DATE,PROCESS_CODE,MCC_CODE,TRACK_2_DATA,TERMINAL_ID,RESPONSE_CODE,RESPONSE_DATA,AUTH_CODE,BILLING_CURRENCY,BILLING_AMOUNT,SETTLEMENT_CURRENCY,SETTLEMENT_AMOUNT,REQUESTED_CURRENCY,REQUESTED_AMOUNT,SETTLEMENT_RATE,SETTLEMENT_DATE,SETTLEMENT_RATE_DATE,SETTLEMENT_FEE,PROCESSING_FEE,POS_ENTRY,POS_DATA,INF_DATA,ORIGINAL_DATA,REPLACEMENT_AMOUNT,REASON_CODE,MERCHANT_ID,MERCHANT_NAME,ECOM_SECURITY_LEVEL,CVC_INVALID,LOCKING_COUNTER,CASHBACK_AMOUNT,EXTERNAL_REF,SYNC_ID,RESPONSE_CODE_INTERNAL,TRANSACTION_ID_OLD,TRANSACTION_LINK_ID_OLD,TRANSACTION_ID,TRANSACTION_LINK_ID,HOSTNAME)
			VALUES (V_BPR_LOG_ID +2,to_timestamp(v_date || '00:00:00','YYYYMMDD HH24.MI.SS'),v_date,'15:51:10','BPR_VISA_ACQ',v_institution_number,'0100',to_date(v_date , 'YYYYMMDD'),to_date(v_date , 'YYYYMMDD'),'228713950502','950502','00000123456','00000000000','999',NULL,NULL,v_card_number,'2212','000000','5999', v_card_number || '=2212221',v_terminal_id,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'978',v_amount,NULL,NULL,NULL,NULL,NULL,'901','050000000000','00DE25:00/DE60.8:00/DE52:N/DE126.10:N/BIN_CNTRY:MT/',NULL,NULL,NULL,'0000000' || v_client_number,'BUNIT MERCHANT           BUNIT CITY   GB',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'6349694e-bc7eeed614-82ebc9',v_transaction_link_id,'RS2\NAFIDH-A557');
			INSERT INTO COS_BPR_DATA (BPR_LOG_ID,RECORD_DATE,BW3_RECORD_DATE,BW3_RECORD_TIME,BPR_TYPE_NAME,INSTITUTION_NUMBER,MESSAGE_TYPE,TRANSACTION_DATE,TRANSMISSION_DATE,RETRIEVAL_REFERENCE,STAN,ACQUIRING_INST_ID,FORWARDING_INST_ID,TRANSACTION_STATUS,NETWORK_ID,BATCH_ID,CARD_NUMBER,EXPIRY_DATE,PROCESS_CODE,MCC_CODE,TRACK_2_DATA,TERMINAL_ID,RESPONSE_CODE,RESPONSE_DATA,AUTH_CODE,BILLING_CURRENCY,BILLING_AMOUNT,SETTLEMENT_CURRENCY,SETTLEMENT_AMOUNT,REQUESTED_CURRENCY,REQUESTED_AMOUNT,SETTLEMENT_RATE,SETTLEMENT_DATE,SETTLEMENT_RATE_DATE,SETTLEMENT_FEE,PROCESSING_FEE,POS_ENTRY,POS_DATA,INF_DATA,ORIGINAL_DATA,REPLACEMENT_AMOUNT,REASON_CODE,MERCHANT_ID,MERCHANT_NAME,ECOM_SECURITY_LEVEL,CVC_INVALID,LOCKING_COUNTER,CASHBACK_AMOUNT,EXTERNAL_REF,SYNC_ID,RESPONSE_CODE_INTERNAL,TRANSACTION_ID_OLD,TRANSACTION_LINK_ID_OLD,TRANSACTION_ID,TRANSACTION_LINK_ID,HOSTNAME)
			VALUES (V_BPR_LOG_ID +3,to_timestamp(v_date || '00:00:00','YYYYMMDD HH24.MI.SS'),v_date,'15:51:10','BPR_VISA_ACQ',v_institution_number,'0110',to_date(v_date , 'YYYYMMDD'),to_date(v_date , 'YYYYMMDD'),'228713950502','950502','00000123456','00000000000','999',NULL,NULL,v_card_number,NULL,'000000',NULL,NULL,v_terminal_id,'00',NULL,v_auth_code,NULL,NULL,NULL,NULL,'978',v_amount,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'00DE25:00/DE62.2:000000000093738/DE52:N/DE44.2: /DE44.5:2/DE44.10:P/DE126.10:N/BIN_CNTRY:MT/DE44.1:V/DE44.3: /',NULL,NULL,NULL,'0000000' || v_client_number,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'6349694e-bc7eeed614-82ebc9',v_transaction_link_id,'RS2\NAFIDH-A557');
			INSERT INTO COS_BPR_DATA (BPR_LOG_ID,RECORD_DATE,BW3_RECORD_DATE,BW3_RECORD_TIME,BPR_TYPE_NAME,INSTITUTION_NUMBER,MESSAGE_TYPE,TRANSACTION_DATE,TRANSMISSION_DATE,RETRIEVAL_REFERENCE,STAN,ACQUIRING_INST_ID,FORWARDING_INST_ID,TRANSACTION_STATUS,NETWORK_ID,BATCH_ID,CARD_NUMBER,EXPIRY_DATE,PROCESS_CODE,MCC_CODE,TRACK_2_DATA,TERMINAL_ID,RESPONSE_CODE,RESPONSE_DATA,AUTH_CODE,BILLING_CURRENCY,BILLING_AMOUNT,SETTLEMENT_CURRENCY,SETTLEMENT_AMOUNT,REQUESTED_CURRENCY,REQUESTED_AMOUNT,SETTLEMENT_RATE,SETTLEMENT_DATE,SETTLEMENT_RATE_DATE,SETTLEMENT_FEE,PROCESSING_FEE,POS_ENTRY,POS_DATA,INF_DATA,ORIGINAL_DATA,REPLACEMENT_AMOUNT,REASON_CODE,MERCHANT_ID,MERCHANT_NAME,ECOM_SECURITY_LEVEL,CVC_INVALID,LOCKING_COUNTER,CASHBACK_AMOUNT,EXTERNAL_REF,SYNC_ID,RESPONSE_CODE_INTERNAL,TRANSACTION_ID_OLD,TRANSACTION_LINK_ID_OLD,TRANSACTION_ID,TRANSACTION_LINK_ID,HOSTNAME)
			VALUES (V_BPR_LOG_ID +4,to_timestamp(v_date || '00:00:00','YYYYMMDD HH24.MI.SS'),v_date,'15:51:10','BPR_HST_ISS',v_institution_number,'0210',to_date(v_date , 'YYYYMMDD'),to_date(v_date , 'YYYYMMDD'),v_retrieval_reference,'093687','00000000999','00000000000','999',NULL,NULL,v_card_number,NULL,'000000',NULL,NULL,v_terminal_id,'00',NULL,v_auth_code,NULL,NULL,NULL,NULL,'978',v_amount,NULL,NULL,NULL,NULL,NULL,'901',NULL,'00DE52:N/DE48.43:N/AVS_DATA:N/DE63.PY:N/DE63.AR:N/DE63.LG:N/DE63.CA:N/DE63.AD:N/DE63.IT:N/DE63.IA:N/DE63.GS:N/BW_DST:009/CVC2_RC:P/MERC_RC:06/',NULL,NULL,NULL,'0000000' || v_client_number,'BUNIT MERCHANT           BUNIT CITY   GB',NULL,NULL,NULL,NULL,v_external_reference,NULL,NULL,NULL,NULL,v_transaction_link_id,v_transaction_link_id,'RS2\NAFIDH-A557');
