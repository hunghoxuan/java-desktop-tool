...
import com.rs2.core.components.property.types.PropertyNode.ChildCount;
import com.rs2.core.components.property.types.PropertyType;
import com.rs2.core.components.property.types.PropertyTypes.ListType;
import com.rs2.core.components.property.types.PropertyTypes.MapType;

@SuppressWarnings("serial")
public class PropertiesTreeTable extends JXTreeTable {
	public List<Integer> readOnlyColumnIndexes = new LinkedList<Integer>();

	public boolean isReadOnlyCell(int column, int row) {
		if (readOnlyColumnIndexes.contains(column))
			return true;
		return false;
	}
	...
	private final class UniqueKeyTreeTableCellEditor extends TreeTableCellEditor {
		...
		private UniqueKeyTreeTableCellEditor(MyTree tree, int row) {
			super(tree);
			this.row = row;
		}
	...
	public TableCellEditor getCellEditor(int row, int column) {
		editingClass = null;
		int modelColumn = convertColumnIndexToModel(column);
		if (isReadOnlyCell(modelColumn, row))
			return null;
		...
		if (modelColumn == getHierarchicalColumn()) {
			...
			MyTree tree = (MyTree) getCellRenderer(0, getHierarchicalColumn());
