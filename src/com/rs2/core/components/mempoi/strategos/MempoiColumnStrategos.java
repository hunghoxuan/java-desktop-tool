 ...
 */
package com.rs2.core.components.mempoi.strategos;

import com.rs2.core.components.mempoi.dao.impl.DBMempoiDAO;
import com.rs2.core.components.mempoi.datapostelaboration.mempoicolumn.MempoiColumnElaborationStep;
import com.rs2.core.components.mempoi.datapostelaboration.mempoicolumn.mergedregions.NotStreamApiMergedRegionsStep;
import com.rs2.core.components.mempoi.datapostelaboration.mempoicolumn.mergedregions.StreamApiMergedRegionsStep;
import com.rs2.core.components.mempoi.domain.MempoiColumn;
import com.rs2.core.components.mempoi.domain.MempoiColumnConfig;
import com.rs2.core.components.mempoi.domain.MempoiColumnConfig.MempoiColumnConfigBuilder;
import com.rs2.core.components.mempoi.domain.MempoiSheet;
import com.rs2.core.components.mempoi.styles.MempoiColumnStyleManager;
import com.rs2.core.components.mempoi.util.RowsUtils;
...
public class MempoiColumnStrategos {
     ...
     *
     * @param mempoiSheet the MempoiSheet from which get GROUP BY's clause
     *                    informations
    ...
    protected List<MempoiColumn> prepareMempoiColumn(MempoiSheet mempoiSheet, ResultSet rs, Workbook workbook) {
        ...
        if (rs != null)
            columnList = DBMempoiDAO.getInstance().readMetadata(rs);
        else {
            if (mempoiSheet.getData() != null) {
                columnList = RowsUtils.getColumnListFromDataList(mempoiSheet.getData());
            }
        }
    ...
    /**
     * get the MempoiColumngConfig list from the MempoiSheet and bind them to the
     * relative MempoiColumns
     *
     * @param mempoiSheet        the MempoiSheet from which read the
     *                           MempoiColumngConfig
     * @param mempoiColumnList   the list of MempoiColumn to which bind the
     *                           MempoiColumnConfiguration
     * @param columnStyleManager the MempoiColumnStyleManager responsible of the
     *                           style management
     */
    private void loadMempoiColumnConfig(MempoiSheet mempoiSheet, List<MempoiColumn> mempoiColumnList,
        ...
        // if a config exist in the map - set it in the relative mempoi column,
        // otherwise set null
        mempoiColumnList.forEach(mempoiColumn -> mempoiColumn.setMempoiColumnConfig(
    ...
    /**
     * if needed adds merged regions configurations to the received list of
     * MempoiColumn
     *
    ...
    private List<MempoiColumn> prepareMergedRegions(MempoiSheet mempoiSheet, List<MempoiColumn> columnList,
        ...
        if (null != mempoiSheet.getMergedRegionColumns()) {
            ...
            Arrays.stream(mempoiSheet.getMergedRegionColumns()).forEach(colName -> {
                ...
                IntStream.range(0, columnList.size())
                        ...
                        .ifPresent(colIndex -> {
                            ...
                            MempoiColumnElaborationStep<?> step = workbook instanceof SXSSFWorkbook
                                    ? new StreamApiMergedRegionsStep<>(columnList.get(colIndex).getCellStyle(),
                                            colIndex, (SXSSFWorkbook) workbook, mempoiSheet)
                                    : new NotStreamApiMergedRegionsStep<>(columnList.get(colIndex).getCellStyle(),
                                            colIndex);
    ...
    /**
     * if needed adds data post elaboration steps configurations to the received
     * list of MempoiColumn
     *
