FUNCTION GET_TOTAL_PENDING_AUTH (VIN_INSTNUMBER IN CAS_ONLINE_ACCOUNT.INSTITUTION_NUMBER%TYPE,
...
BEGIN
 ...
 IF CUR_ACCTSHARED%FOUND THEN
   SELECT NVL(
              LTRIM(
                     TO_CHAR(
                              SUM( TO_NUMBER(NVL(ACCOUNT_AMOUNT_NET,'0')) * DECODE(DR_CR_INDICATOR, '001', -1, 1)
                                 )
                            )
                   )
             ,0) INTO V_RETURN
 ...
 ELSE
  ...
  IF CUR_ACCTALLOCATED%FOUND THEN
     SELECT NVL(LTRIM(TO_CHAR(SUM(TO_NUMBER(NVL(ACCOUNT_AMOUNT_NET,'0')) * DECODE(DR_CR_INDICATOR,'001',-1,1)))),0) INTO V_RETURN
     FROM INT_ONLINE_TRANSACTIONS
     WHERE INSTITUTION_NUMBER = VIN_INSTNUMBER
     AND TRANSACTION_CLASS = '010'
     AND TRANSACTION_STATUS = '005'
