package com.java.modules.files.isoparser.containers.mastercard;
...
public class IPMProbe {
    public AppConfig.Encoding encoding = null;
    public AppConfig.Container container = null;
    ...
    public void probe(String path) {
        ...
        try {
            ...
            if (probe1014)
                Trace.log("PROBE", "File size is multiple of 1014. Could be MC 1014 aligned layout");
            ...
            // byte[] hdrRDW = new byte[] { (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte)
            // 0x4A };
            // byte[] hdrMF = new byte[] { (byte) 0x00, (byte) 0x4E, (byte) 0x00, (byte)
            // 0x00 };
            ...
            if (Arrays.equals(hdr, hdrASCII)) {
                ...
                encoding = AppConfig.Encoding.ASCII;
            } else if (Arrays.equals(hdr, hdrEBCDIC)) {
                Trace.log("PROBE", "Detected EBCDIC encoding");
                encoding = AppConfig.Encoding.EBCDIC;
            }

            if (isRdw && probe1014) {
                Trace.log("PROBE",
                        "RDW found and file size is multiple of 1014. Will check for MC 1014 blocked layout");
            ...
            if (is1014) {
                ...
                container = AppConfig.Container.FIXED1014;
            } else if (isRdw) {
                Trace.log("PROBE", "Detected container RDW");
                container = AppConfig.Container.RDW;
            } else {
                Trace.log("PROBE", "Detected container NONE");
                container = AppConfig.Container.CLEAN;
            }
