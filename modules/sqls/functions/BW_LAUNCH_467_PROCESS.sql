procedure bw_launch_467_process(
...
begin
  ...
  --
  v_params := BW_PROCESS_CONTROL.GET_PROCESS_PARAM_STRING_LIST(p_instno, '467', v_postdate);
  --
  if v_params != '[# NO DATA FOUND! #]' then
     if instr(v_params, '|p_rec_type=>'''||p_rec_type) > 0 then
     --
        bw_process_control.run_process(
            p_InstitutionNumber_IN  => p_instno,
            p_ProcessName_IN        => '467',
            p_ProcessParameters_IN  => 'p_svc_cont=>''999''|p_rec_type=>'''|| p_rec_type ||'''',   --hard-code to do all merchant applications for any svc contract!!
