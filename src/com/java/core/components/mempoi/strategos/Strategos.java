...
import com.java.core.components.mempoi.builder.MempoiSheetMetadataBuilder;
...
import com.java.core.components.mempoi.domain.MempoiReport;
import com.java.core.components.mempoi.domain.MempoiSheet;
import com.java.core.components.mempoi.domain.MempoiSheetMetadata;
import com.java.core.components.mempoi.exception.MempoiException;
import com.java.core.components.mempoi.manager.ConnectionManager;
import com.java.core.components.mempoi.manager.FileManager;
...
public class Strategos {
     ...
     *
     * @param mempoiSheetList the List of MempoiSheet containing the
     *                        PreparedStatement to execute to export data into
    ...
    /**
     * starting from export PreparedStatement prepares the MempoiReport for battle!
     * writes exported data to the received
     * file parameter
     *
     * @param mempoiSheetList the List of MempoiSheet containing the
     *                        PreparedStatement to execute to export data into
     *                        mempoi report and eventually the sheet's name
     * @param fileToExport    the destination file (with path) where write exported
     *                        data
    ...
    private MempoiSheetMetadata generateSheet(MempoiSheet mempoiSheet) {
        ...
        if (mempoiSheet.getPrepStmt() != null) {
            // read data from db
            rs = DBMempoiDAO.getInstance().executeExportQuery(mempoiSheet.getPrepStmt());
        }
        ...
        try {
            ...
            rowCounter = this.dataStrategos.addSimpleTextHeader(mempoiSheet, rowCounter, firstCol,
                    mempoiSheetMetadataBuilder);
     ...
     * @param columnList  the list of MempoiColumn from which read data
     *                    configuration
     ...
     * @para mempoiSheetMetadataBuilder the mempoiSheetMetadataBuilder in which
    ...
    private AreaReference createSheetData(ResultSet rs, List<MempoiColumn> columnList, MempoiSheet mempoiSheet,
        ...
        if (rs != null)
            localRowCounter = this.dataStrategos.createDataRows(mempoiSheet.getSheet(), rs, columnList, rowCounter,
                    firstCol);
        else
            localRowCounter = this.dataStrategos.createDataRows(mempoiSheet.getSheet(), mempoiSheet.getData(),
                    rowCounter,
                    firstCol);
    ...
    /**
     * opens the temp saved report file assigning it to the class workbook variable,
     * then evaluate all available cell
     * formulas
     *
    ...
    private void openTempFileAndEvaluateCellFormulas(File tmpFile) {
        ...
        try {
            ...
            this.workbookConfig.setWorkbook(Utils.createWorkbook(tmpFile));
