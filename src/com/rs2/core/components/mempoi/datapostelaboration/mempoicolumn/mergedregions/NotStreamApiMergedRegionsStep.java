...
package com.rs2.core.components.mempoi.datapostelaboration.mempoicolumn.mergedregions;
...
public class NotStreamApiMergedRegionsStep<T> implements MempoiColumnElaborationStep<T> {
    ...
    /**
     * the index of the containing MempoiColumn in the list of columns of the owner
     * sheet
     */
    private int mempoiColumnIndex;

    /**
     * the MergedRegionsManager containing the logic (shared by
     * NotStreamApiMergedRegionsStep and StreamApiMergedRegionsStep)
     */
    private MergedRegionsManager<T> mergedRegionsManager;

    /**
     * if MempoiColumn has to by merged, this variable contains MergedRegions'
     * limits
     */
    ...
    public void execute(MempoiSheet mempoiSheet, Workbook workbook) {
        ...
        if (!this.mergedRegionsLimits.isEmpty()) {
            ...
            // clone the MempoiColumn's style into another one created by the current
            // workbook
            CellStyle newStyle = workbook.createCellStyle();
            newStyle.cloneStyleFrom(this.cellStyle);

            // for each pair add a MergedRegion
            this.mergedRegionsLimits.forEach(pair -> this.mergedRegionsManager.mergeRegion(sheet, newStyle,
                    pair.getLeft(), pair.getRight(), this.mempoiColumnIndex));
        }
    }
}
