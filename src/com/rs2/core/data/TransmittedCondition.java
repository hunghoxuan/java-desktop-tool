...
public class TransmittedCondition implements Comparable<TransmittedCondition> {
	...
	public static TransmittedCondition cloneCondition(TransmittedCondition toClone) {
		TransmittedCondition clone = new TransmittedCondition(toClone.getIdentifier(), toClone.getValue(),
				toClone.getDepth(), toClone.inheritanceIsNotApplicable());
		return clone;
	}
	...
	public String toString() {
		// return "identifier=" + identifier + ", value=" + value + ", depth=" +
		// depth+", inheritanceIsNotApplicable="+inheritanceIsNotApplicable;
		return identifier + ": " + value.toLowerCase().replace("in (", "").replace(")", "").replace("'", "");
	}
