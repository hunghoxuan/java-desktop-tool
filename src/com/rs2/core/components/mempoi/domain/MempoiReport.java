...
public class MempoiReport {
    ...
    /**
     * contains the filename (absolute path) where the MempoiReport has been saved
     * (if exporting to file)
     */
    private String file;
    /**
     * contains the byte array containing MempoiReport data (if exporting to byte
     * array)
     */
    private byte[] bytes;
    /**
     * the map of sheets' metadata the key is the sheet index the value is the
     * metadata object
     */
    ...
    /**
     * insert a MempoiSheetMetadata relative to the sheet at the index identified by
     * the first param
     *
    ...
    public MempoiReport addMempoiSheetMetadata(Integer sheetIndex, MempoiSheetMetadata mempoiSheetMetadata) {
        ...
        if (sheetIndex < 0) {
            throw new MempoiException(
                    String.format("The index of a sheet can't be minor than 0. %d received", sheetIndex));
        }
