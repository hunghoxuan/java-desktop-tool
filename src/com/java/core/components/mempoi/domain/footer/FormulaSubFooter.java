...
public abstract class FormulaSubFooter implements MempoiSubFooter {
    ...
    @Override
    public void setColumnSubFooter(Workbook workbook, List<MempoiColumn> mempoiColumnList, CellStyle subFooterCellStyle,
            int firstDataRowIndex, int lastDataRowIndex, int colOffset) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (!mc.getColumnName().equalsIgnoreCase("id") && MempoiColumnStyleManager.isNumericType(mc.getType())) {
                ...
                mempoiColumnList.get(i).setSubFooterCell(new MempoiSubFooterCell(
                        this.getFormula(colLetter, firstDataRowIndex, lastDataRowIndex), true, subFooterCellStyle));
     ...
     * @param firstDataRowIndex the first data row index (headers and subheaders are
     *                          not counted)
     * @param lastDataRowIndex  the last data row index (footers and subfooters are
     *                          not counted)
