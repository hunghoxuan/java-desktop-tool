...
import com.java.core.components.mempoi.domain.datatransformation.DataTransformationFunction;
...
public class DataStrategos {
     ...
     * @param rowOffset                  the row offset identifying the current row
     *                                   to use
    ...
    /**
     * creates data rows for the received Sheet and populates them with ResultSet
     * data
     *
     * @param sheet      the sheet to which add data rows
     * @param rs         the ResultSet from which read data to write in the Sheet
     * @param columnList the List of MempoiColumn from which read the data
     *                   configuration (data type, format, etc)
     ...
     * @return the incremented row couter
     */
    protected int createDataRows(Sheet sheet, List<List<String>> data, int rowCounter,
            int colOffset) {

        try {
            List<MempoiColumn> columnList = null;
            int colListLen = 0;
            int rowIndex = 0;
            ...
            for (List<String> item : data) {
                logger.debug("creating row {}", rowCounter);
                if (colListLen == 0) {
                    columnList = RowsUtils.getColumnListFromDataList(data);
                ...
                for (int colNum = colOffset, mcIndex = 0; mcIndex < colListLen; colNum++, mcIndex++) {
                    ...
                    Object value = item.get(mcIndex);
                    value = this.getValueOrNull(value);

                    Object cellValue;
                    cellValue = value;

                    if (mempoiColumn.getMempoiColumnConfig() != null) {
                        Optional<DataTransformationFunction<?, ?>> optDataTransformationFunction = mempoiColumn
                                .getMempoiColumnConfig().getDataTransformationFunction();
                        if (optDataTransformationFunction.isPresent()) {
                            cellValue = optDataTransformationFunction.get().execute(null, value);
                        } else {
                            cellValue = value;
                        }
                        // sets value in the cell
                        if (hasCellValueToBeSet(null, optDataTransformationFunction)) {
                            if (mempoiColumn.getCellSetValueMethod() != null)
                                mempoiColumn.getCellSetValueMethod().invoke(cell, cellValue);
                        }
                        // analyze data for mempoi column's strategy
                    } else // sets value in the cell
                    {
                        if (mempoiColumn.getCellSetValueMethod() != null)
                            mempoiColumn.getCellSetValueMethod().invoke(cell, cellValue);
                    }
    ...
    /**
     * receive an object read from the DB and return the same value or null
     * depending on the original value in the DB
     ...
     * and on the workbookConfig.nullValuesOverPrimitiveDetaultOnes property
     *
     * @param resultSet                     the result set from which check if the
     *                                      read value was null
     * @param optDataTransformationFunction the optional DataTranformationFunction
     *                                      to apply to the read value
